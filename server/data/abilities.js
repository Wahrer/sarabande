...
let BattleAbilities = {
  ...
  "slushforce": {
		desc: "If Hail is active, this Pokemon's Fire-, Ice-, and Water-type attacks have their power multiplied by 1.333. This Pokemon takes no damage from Hail.",
		shortDesc: "This Pokemon's Fire/Ice/Water attacks do 1.333x in Sandstorm; immunity to it.",
		onBasePowerPriority: 8,
		onBasePower(basePower, attacker, defender, move) {
			if (this.field.isWeather('hail')) {
				if (move.type === 'Fire' || move.type === 'Ice' || move.type === 'Water') {
  ...
  "synthesap": {
		...
		shortDesc: "Recovers 1/8th of all direct HP damage dealt.",
  ...
  "overglow": {
		desc: "This Pokemon's light-based attacks have their power multiplied by 1.5.",
		shortDesc: "This Pokemon's light-based attacks have 1.5x power.",
		onBasePowerPriority: 8,
		onBasePower(basePower, attacker, defender, move) {
			if (move.flags['lumen']) {
				return this.chainModify(1.5);
  ...
  "socialworker": {
    desc: "This Pokemon's high critical-hit ratio and hammer-based attacks have their power multiplied by 1.5.",
		shortDesc: "Hammer-based and innately high-critting moves have 1.5x power.",
    onBasePowerPriority: 8,
    onBasePower(basePower, attacker, defender, move) {
      if (move.flags['swerk']) {
        return this.chainModify(1.5);
  	...
  	"umbrage": {
		shortDesc: "On switch-in, this Pokemon summons Moony Night.",
		onStart(source) {
			/*
      for (const action of this.queue) {
				if (action.choice === 'runPrimal' && action.pokemon === source && source.template.speciesid === 'groudon') return;
			...
			this.field.setWeather('moonynight');
		},
		id: "umbrage",
		name: "Umbrage",
		rating: 4.5,
	...
	"nightcrawler": {
		shortDesc: "If Moony Night is active, this Pokemon's Speed is doubled.",
		onModifySpe(spe) {
			if (this.field.isWeather(['moonynight'])) {
				return this.chainModify(2);
			}
		},
		id: "nightcrawler",
  ...
  "lunarpower": {
		desc: "If Moony Weather is active, this Pokemon's Special Attack is multiplied by 1.5 and it loses 1/8 of its maximum HP, rounded down, at the end of each turn.",
		shortDesc: "If Moony Weather is active, this Pokemon's Sp. Atk is 1.5x; loses 1/8 max HP per turn.",
		onModifySpAPriority: 5,
		onModifySpA(spa, pokemon) {
			if (this.field.isWeather(['moonynight'])) {
				return this.chainModify(1.5);
