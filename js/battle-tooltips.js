...
if(isZ){
var _item=this.battle.dex.getItem(serverPokemon.item);
if(_item.zMoveFrom===move.name){
move=this.battle.dex.getMove(_item.zMove);
...
var _moveid;
if(_item.zMoveType){
_moveid=BattleTooltips.zMoveTable[_item.zMoveType];
}else{
...
if(_item2)itemText='<small>Item:</small> '+_item2+itemEffect;
}
...
var _ability=this.getAllyAbility(active);
if(_ability==='Victory Star'){
...
if(serverPokemon.item){
var _item3=Dex.getItem(serverPokemon.item);
if(move.id==='fling'&&_item3.fling){
value.itemModify(_item3.fling.basePower);
}
if(move.id==='naturalgift'){
value.itemModify(_item3.naturalGift.basePower);
}
}
...
var _species2=clientPokemon.getSpecies()||(serverPokemon==null?void 0:serverPokemon.species)||'';
var template=this.battle.dex.getTemplate(_species2);
...
var _ability2=abilityData.baseAbility||abilityData.ability;
if(_ability2)text='<small>Ability:</small> '+Dex.getAbility(_ability2).name;
}else{
...
var _hp=evs['hp']||0;
stat=this.getStat('hp',set,_hp,1);
...
if(hpDivisibility){
while(_hp<252&&evTotal<508&&!(stat%hpDivisibility)!==hpShouldBeDivisible){
_hp+=4;
stat=this.getStat('hp',set,_hp,1);
evTotal+=4;
}
while(_hp>0&&!(stat%hpDivisibility)!==hpShouldBeDivisible){
_hp-=4;
stat=this.getStat('hp',set,_hp,1);
evTotal-=4;
}
while(_hp>0&&stat===this.getStat('hp',set,_hp-4,1)){
_hp-=4;
evTotal-=4;
}
if(_hp||evs['hp'])evs['hp']=_hp;
}
